...
impl StackItem {
    ...
    pub fn from_input_and_step(
        input: &str,
        step_arg_type: StepArgType
    ) -> Result<StackItem> {
        match step_arg_type {
            StepArgType::Int => {
                Ok(StackItem::Num(input.to_string()))
            }
            StepArgType::Slice => {
...
impl Serialize for DeserializableCell {
    ...
    where
        S: Serializer,
    {
        let bytes = BagOfCells::from_root(
            self.0.as_ref().clone()
        )
            .serialize(false)
...
impl TonSingleVerifier {
    ...
    async fn do_intermediate_step(
        ...
        let response = self.make_call(address, &step.method, vec![stack_item]).await?;
    ...
    async fn verify(&self, auth_contract_id: &str, args: VerifyArgs) -> Result<bool> {
        ...
        if inputs.len() != validation_schema.len() {
            bail!(
                ...
                validation_schema.len(), inputs.len()
            );
        }
        ...
        let is_verify_success = match input_and_step.split_last() {
            Some((last, rest)) => {
                ...
                self.do_final_step(&current_address, last_input, last_step).await?
            }
...
mod tests {
    ...
    use anyhow::Result;
    use tonlib_core::TonAddress;
    use crate::internals::SingleVerifier;
    ...
    async fn test_ton_call() -> Result<()> {
        ...
        let response = verifier.make_call(
            &addr,
            method,
            stack,
        ).await?;
    ...
    #[tokio::test]
    async fn single_verifer() -> Result<()> {
        let server = "https://toncenter.com/api/v3".to_string();
        let client = Arc::new(reqwest::Client::new());
        let verifier = TonSingleVerifier::new(client, server);
